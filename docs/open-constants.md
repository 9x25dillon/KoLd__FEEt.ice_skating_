...
- **Stage B2, the feet** (`footMode` 1, with `slipMode` 1; `solver.ts` `legTurns`, `footTangent`, `slipSolveFeet`)
  ...
  - Model limit: with `pitchMode` 0 (every setup) there is no fore-aft pendulum — a snowplow on outside edges catches both and stops dead in 1 s without pitching the skater forward. See `pitchMode` below

- **`pitchMode`** — the fore-aft pendulum (`solver.ts` `pitchTick`; big_reffg.txt §3.5, the lateral pendulum's mirror along the support blade). **0 in every setup** until the operator switches it on
  ...
  - Measured with it forced on in every setup (not pinned): the dig (`test/torque.test.ts`) no longer winds the body — the ankle cannot put the contact on the toe at once, since leaning toward the toe first moves it toward the heel; the hockey stop still stands but takes 6.3 s after the rise instead of 3.0; the heel/toe carve and toe-pivot tests move with the contact's lag
