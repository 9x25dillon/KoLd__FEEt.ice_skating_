...
//       `crossSide`), turns, twizzles and spins (`move`, `turn`, `spin`,
//       `moveDone`, `movesHeld`, `flips`, `spinCarry`; SkatingInput `turn`,
//       `twizzle`, `spin`). With movesMode 0 — every preset — nothing
...
export function parseReplay(json: string): Replay {
  ...
  for (let i = 0; i < root.frames.length; i++) {
    ...
    const input = object(frame.input, `${path}.input`,
      ["lean", "knee", "weight", "pitch", "leanSplit", "push", "brake", "carriage", "toe", "turn", "twizzle", "spin"]);
    ...
    for (const key of ["push", "brake", "toe", "turn", "twizzle", "spin"]) {
