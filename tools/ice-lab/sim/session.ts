...
/** Stick deflection that counts as the player having done something. */
const INTENT = 0.1;
...
export interface SessionSummary {
   ...
   *
   * In the rig, getting up again is the reset, so this measures fall -> back on
   * their feet, or fall -> a fresh push, whichever the tester does first. A fall
   * never got up from contributes no sample rather than an infinite one, and a
   * session with no samples reports -1 rather than 0 — an unmeasured retry and
   * an instant one are not the same number.
   */
  medianTimeToRetrySeconds: number;
  /** Seconds spent on the ice after a fall. 0 means they never went down. */
  downSeconds: number;
...
export class SessionMeter {
  ...
  sample(s: SkaterState, input: SkatingInput, events: EdgeEvent[], dt: number): void {
    ...
    if (s.fallen) {
      this.downTicks++;
      // A stick still held from before the fall is not a new input — that
      // reads as an instant retry every time, which is how this metric first
      // came out as a confident zero. It takes a FRESH push.
