// tools/ice-lab/sim/moves.ts — the moves: turns first.
//
...
import { rotate, normalizeOr, len, mul, sin, sign } from "./math.ts";
...
export function newTurn(): TurnState {
  return {
    ...
    foot: FOOT.Right, exitFoot: FOOT.Right, cusped: false, kind: TURN_KIND.ThreeTurn,
    fromCode: EDGE_CODE_NONE, entrySpeed: 0,
  };
}

export function noMove(): MoveResult {
  return { tick: -1, kind: MOVE.None, detail: 0, fromCode: EDGE_CODE_NONE, toCode: EDGE_CODE_NONE, speedLost: 0 };
}
 ...
 * ice, one blade on an edge, moving. The body turns the way the path already
 * is — into the curve. Returns whether it started.
 */
export function turnStart(s: SkaterState, p: Params): boolean {
  ...
  if (!b.inContact || Math.abs(b.tilt) < p.flatThreshold || speed < p.turnMinSpeed) return false;
  const T = s.turn;
  const pathSense = sign(b.tilt) * sign(b.longSpeed);
  if (pathSense === 0) return false;
  T.t = 0;
  T.swept = 0;
  T.dir = pathSense;
...
/** The foot a turn in progress has its weight on. */
export function turnLoadFoot(s: SkaterState): Foot {
  return s.turn.cusped ? s.turn.exitFoot : s.turn.foot;
}

/**
 * The heading the pendulum is measured against during a turn: the direction of
 * travel, forward before the cusp and backward after it.
 */
export function turnFrame(s: SkaterState): { x: number; y: number } {
  const T = s.turn, v = len(s.vel);
  const sense = T.cusped ? -T.entryDir : T.entryDir;
  return v > 1e-6 ? mul(s.vel, sense / v) : s.blade[turnLoadFoot(s)].tangent;
}
 ...
 * lateral acceleration the body feels, for the pendulum, and whether this tick
 * was the cusp.
 */
export function turnPivot(
  s: SkaterState, p: Params, dt: number, weightR: number,
): { lat: number; cusp: boolean } {
  const T = s.turn;
  const g = p.gravity;
  T.t += dt;
  ...
  let speed = len(s.vel);
  if (speed > 1e-6) {
    const scrub = T.cusped && T.kind === TURN_KIND.Mohawk ? p.mohawkScrub : 1;
    const dv = (p.muTurn * g * Math.abs(sin(T.swept)) * scrub + p.muGlide * g
  ...
  // The end: aligned with the path again, the other way round.
  if (T.swept >= Math.PI) {
    const frame = speed > 1e-6 ? mul(s.vel, sense / speed) : s.heading;
    ...
    const r = s.moveDone;
    r.tick = s.tick; r.kind = MOVE.Turn; r.detail = T.kind; r.fromCode = T.fromCode;
    r.toCode = s.blade[T.exitFoot].code; r.speedLost = T.entrySpeed - speed;
  }
  return { lat: T.pathRate * speed * sense, cusp };
}

/** After classification on the cusp tick: say which turn it was, and onto what. */
...
/** The rotation a turn left in the body drains away. */
