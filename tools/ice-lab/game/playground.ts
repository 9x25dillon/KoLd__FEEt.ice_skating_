  ...
  sample(s: SkaterState,dt:number) {
    ...
    this.flakes=this.flakes.map((cool,i)=> {
      ...
      if(ground && Math.hypot(s.pos.x-SNOWFLAKES[i].x,s.pos.y-SNOWFLAKES[i].y)<2) {
        this.reward(100,"Snowflake +100");this.flow=Math.min(100,this.flow+8);return 18;
      }
      return 0;
    });
    this.pucks.forEach((p,i)=> {
      ...
      if(TARGETS.some(t=>Math.hypot(t.x-p.x,t.y-p.y)<2.4)) {this.reward(500,"GOAL! +500");p.cooldown=5;}
      if(Math.abs(p.x)>27 || p.y< -9 || p.y>45) {p.cooldown=3;}
    });
    if(s.landed.tick>=0 && this.landed!==s.landed.tick) {
      this.landed=s.landed.tick;
      if(!s.landed.fall) this.reward(300+Math.round(s.landed.turned)*150,`LANDED · ${s.landed.turned.toFixed(1)} rotations`);
    }
    if(s.move===MOVE.Spin && s.spin.swept>=Math.PI*4 && !this.spinning) {this.reward(300,"SICK SPIN! +300");this.spinning=true;}
    if(s.move!==MOVE.Spin) this.spinning=false;
  }
  private reward(points:number,message:string) {this.score+=points;this.message=message;this.toast=3;}
}
